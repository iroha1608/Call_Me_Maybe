...
3. [⛏追加要件](#3-追加要件)
	...
	6. [使用例](#3-6-使用例)
4. [🎁ボーナス](#4-ボーナス)
	1. [高度なエラー回復メカニズム](#4-1-高度なエラー回復メカニズム)
	2. [パフォーマンスの最適化（キャッシュ、バッチ処理）](#4-2-パフォーマンスの最適化（キャッシュ、バッチ処理）)
	3. [包括的なテストスイート](#4-3-包括的なテストスイート)
	4. [生成プロセスの可視化](#4-4-生成プロセスの可視化)
	5. [制約付きデコードとのエンコードおよびデコードの統合方法のデモンストレーション](#4-5-制約付きデコードとのエンコードおよびデコードの統合方法のデモンストレーション)
5. [🌈リソース](#5-リソース)
	1. [参考URL](#5-1-参考URL)
...
```
- このコマンドは、デフォルトで`data/input/`ディレクトリから入力ファイルを読み取り、`data/output/`ディレクトリに結果を書き出します。  
- 最終的な出力は`data/output/function_calling_results.json`という単一のJSONファイルです。  
- 入力された各自然言語プロンプトに対し、以下のキーを厳密に持つJSONオブジェクトの配列を生成します。(PDFより)  
```
...
- JSON構造化を強制キューによる管理に変更するにあたり、それはハードコーディングに当たらないかと悩みましたがそもそも指定された項目である"name"や"parameters"、"type"などの事項は必須で取得しなければならず、今回大事なのは制約付きでコーディングの理解とLLMの推論能力を最大限に活用することだと判断し導入しました。  
- '\"'の処理について、もしプロンプトに'\"'が含まれていた場合、LLMに渡す時にエスケープが消費されただのダブルクォーテーションとして出力してしまい、FSMが壊れる場面がありました。
	LLMに渡す前にプロンプト中に'"'が含まれている場合、それをエスケープしてから渡すようにしました。  
...
- 以下の5つについて実装しました。
	- 高度なエラー回復メカニズム
	- パフォーマンスの最適化（キャッシュ、バッチ処理）
	- 包括的なテストスイート
	- 生成プロセスの可視化
	- 制約付きデコードとのエンコードおよびデコードの統合方法のデモンストレーション
	
### 4-1. 高度なエラー回復メカニズム

- ConstraintFilterでフィルタリングがクラッシュした際に元のLogitにフォールバックする機構
- 不完全なJson文字列でもパースエラーを飲み込んで生成を継続するtry-exceptの制御フロー
- Errorが発生しても全体を落とさない設計。
- カスタムエラーの作成。どこでエラーが起きたかわかりやすく操作。

### 4-2. パフォーマンスの最適化（キャッシュ、バッチ処理）

- カレントテキスト解析時のキャッシュ、再開処理
- Trie木を使用したキャッシュ処理
- バッチ処理

### 4-3. 包括的なテストスイート

- pytest, unittest.mockを使い、外部I/o、LLM推論を をモック化したユニットテスト群(test/)として構築。

### 4-4. 生成プロセスの可視化

- 各トークンのLogitsに制約をかける前と、制約付きデコードを実施した後のトークンの可視化、また、それに併せて現在出来上がっている文字列を表示。
- エスケープシークエンスを用いて画面描画を操作。

### 4-5. 制約付きデコードとのエンコードおよびデコードの統合方法のデモンストレーション
- src/engine.pyの中で自己回帰性ループ、これらがパイプラインとしてどう連携するかを体現。
- ConstraintsFilter内で、強制キュー使用時に特定のスキーマをデコードする。
...
- 42のプロジェクトページより、世界のレビュー履歴最新300件取得スクリプトの作成相談。  
->それを元にしたGemini CLIによるマークダウンファイルの作成。  
- 制約付きデコードにおける出力形式を課題に合わせるかどうか、解決策の壁打ち。  
- エッジケースの案出し。  
- コードリファクタリングの相談。　　
- README.md作成における自分が行った内容のまとめ。目次作成。
